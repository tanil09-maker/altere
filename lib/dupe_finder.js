...
function isAllowedStore(sourceText, storeSet) {
  ...
  for (const store of allowed) {
    ...
    // Starts with store + space
    if (n.startsWith(s + ' ')) return true;
    // Ends with space + store
    if (n.endsWith(' ' + s)) return true;
    // Word boundary match
    try {
      if (new RegExp(`\\b${escapeRegex(s)}\\b`).test(n)) return true;
    } catch { /* skip bad regex */ }
    // Domain match: "zara.com", "hm.com"
    const slug = s.replace(/[&\s]/g, '');
    if (n.includes(slug + 'com') || n.includes(slug + '.com') || n.includes(slug + 'stores')) return true;
  }
...
export async function searchDupes(productInfo, regionOptions = {}) {
  ...
  console.log(`[dupe_finder] Generated ${queries.length} queries:`, queries);

  // Step 2: Search all queries in PARALLEL
  const searchPromises = queries.map(query =>
    searchGoogleShopping(query, { num: 30, ...serpApiParams })
      .catch(err => { console.error(`[dupe_finder] "${query}" failed:`, err.message); return []; })
  );
  ...
  console.log(`[dupe_finder] ${candidates.length} total -> ${filtered.length} whitelisted`);

  // Step 5: Fallback if too few
  if (filtered.length < 5) {
    console.log('[dupe_finder] Too few results, running broad fallback query');
    ...
    console.log(`[dupe_finder] After fallback: ${filtered.length} whitelisted`);
  }
