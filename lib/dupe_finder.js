...
// High-street stores we search for dupes
const DUPE_STORES = [
  'zara', 'h&m', 'hm.com', 'mango', 'asos', 'cos', '& other stories',
  'other stories', 'massimo dutti', 'uniqlo', 'pull&bear', 'pull and bear',
  'bershka', 'stradivarius', 'arket', 'weekday'
];

function isHighStreetStore(source) {
  const s = (source || '').toLowerCase();
  return DUPE_STORES.some(store => s.includes(store));
}
...
export async function searchDupes(productInfo) {
  ...
  for (const query of queries) {
    try {
      const results = await searchGoogleShopping(query, { num: 15 });
