...
export async function searchGoogleShopping(query, options = {}) {
  ...
  return results.map(item => ({
    title: item.title || '',
    // Keep SerpAPI's already-localized price string (e.g. "€129", "£99") instead of
    // forcing "$". The frontend re-formats with the region symbol when extracted_price
    // exists; this only matters as a fallback, so preserve the correct currency.
    price: item.price || (item.extracted_price != null ? String(item.extracted_price) : ''),
